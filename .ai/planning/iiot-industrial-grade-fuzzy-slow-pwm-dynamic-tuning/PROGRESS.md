...
| F6 | Implement `createPendingCommand()` tạo desired/audit, publish desired retained QoS 1 và ghi thời điểm publish. | `[ ] QA Review` | Actor phải từ JWT đã verify, không từ client. Dùng command idempotency key, full config snapshot và ownership check. Làm rõ xử lý lỗi publish (không được để row PENDING mồ côi không audit/retry strategy); không auto-apply. |
