...
|---|---|---|---|
| A1 | Định nghĩa constants cho MQTT tuning desired/reported và pattern subscribe; tenant lấy từ `IOT_TENANT`. | `[ ] QA Review` | **QA REJECTED 2026-07-23:** `AppConfigService` không dùng cùng validation segment với topic builder và không giới hạn tenant tối đa 50 ký tự; dùng validator dùng chung/fail closed trước khi inject vào MQTT/ACL. |
...
| A5 | Thêm `INFLUXDB_ANALYTICS_BUCKET` vào cấu hình backend và tạo script provision bucket analytics idempotent. | `[ ] QA Review` | Áp dụng **Cấu hình qua environment**: không hard-code `mushroom_analytics`; script phải idempotent, validate/escape toàn bộ giá trị environment trước khi đưa vào URL hoặc JSON, và retention configurable. Không sửa lock/generated file; ghi hướng dẫn vận hành/recovery tối thiểu. |
...
| C5 | Implement đọc/ghi NVS two-slot: verify CRC/readback, chọn generation mới nhất, wear-level slot và fallback defaults. | `[ ] QA Review` | **QA REJECTED 2026-07-23:** `loadDurableReceipt()` trong storage đã bỏ UUID char-by-char validation sau khi CRC/NUL pass; khôi phục validation bounded trước khi hydrate receipt và thêm regression CRC-valid malformed UUID. |
| C6 | Khai báo `g_tuning_config_queue` depth 1 cho `DynamicTuningParams`. | `[ ] QA Review` | Queue là ranh giới ownership Core 0 → Core 1; chỉ truyền POD by value. Không dùng mutex chia sẻ control state; không đổi queue của baseline/config sync hiện hữu. |
...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] QA Review` | **QA REJECTED 2026-07-23:** `extractCommandId()` dùng quét chuỗi heuristic nên có thể không nhận ra hoặc nhận nhầm `command_id` JSON hợp lệ (key escaped/nested/prefix); command vẫn được persist/dispatch nhưng không reserve outbox, rồi `enqueuePendingReport()` thất bại khi outbox đầy làm mất ACK. Dùng parse/validation có cấu trúc cho đúng root `command_id`, reserve slot theo ID canonical trước mọi persist/dispatch, kiểm tra mọi return của enqueue; thêm regression cho key escaped/nested/prefix khi outbox còn đúng một chỗ và khi đầy. |
