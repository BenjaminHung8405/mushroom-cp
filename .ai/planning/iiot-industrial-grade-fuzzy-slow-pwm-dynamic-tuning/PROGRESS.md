...
| C5 | Implement đọc/ghi NVS two-slot: verify CRC/readback, chọn generation mới nhất, wear-level slot và fallback defaults. | `[ ] In Progress` | **QA REJECTED 2026-07-21 (lần 2):** `saveDurableReceipt()` gọi `std::strcmp(read_rec.command_id, command_id)` trước khi xác nhận mảng `command_id[37]` đọc từ NVS có NUL terminator (`tuning_config_manager.cpp:456`). NVS là dữ liệu không tin cậy; record CRC-valid nhưng không có NUL có thể làm đọc vượt biên/UB. So sánh bounded `memcmp` với record expected, verify CRC trước parse chuỗi, và tại `loadDurableReceipt()` kiểm tra NUL trong phạm vi 37 byte trước UUID validation. Thêm regression cho receipt CRC-valid nhưng command_id không NUL. |
...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] In Progress` | **QA REJECTED 2026-07-21 (lần 2):** Cần xóa executable generated `mushroom-iot-firmware/run_tests_audit` đã bị commit trong thay đổi D4 và không commit binary test artifact. Giữ source/test runner có thể tái lập, thêm artifact vào `.gitignore` nếu cần. |
