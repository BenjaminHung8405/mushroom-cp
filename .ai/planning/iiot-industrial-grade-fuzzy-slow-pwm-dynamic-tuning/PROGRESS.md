...
| H4 | Implement `ControlAnalyticsService.checkCoverageGate()` kiểm tra coverage, trusted samples và revision. | `[ ] In Progress` | **Gatekeeper Pattern (CORR-S2-02):** `<80%` trả `COVERAGE_BELOW_80_PERCENT`; mixed quality + `<100` samples trả `INSUFFICIENT_TRUSTED_SAMPLES`; revision null trả `CONFIG_REVISION_UNAVAILABLE`. Bất kỳ gate nào fail đều ngăn recommender chạy. |
| H5 | Implement `ControlAnalyticsService.checkDeviceOnline()` kiểm tra telemetry last-seen trong 5 phút. | `[ ] In Progress` | **Fail-Closed Availability Check:** Truy vấn timestamp telemetry mới nhất; chỉ trả online khi `lastSeen > now - 5 minutes`. Lỗi truy vấn/không có telemetry phải được coi là offline, không phát advisory cho device không thể xác nhận. |
