...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] QA Review` | **QA REJECTED 2026-07-23:** `retryPendingDispatch()` vẫn có thể commit/apply command rồi `enqueuePendingReport()` thất bại khi outbox đầy, làm mất ACK. Reserve capacity trước dispatch hoặc giữ pending dispatch đến khi ACK đã được nhận vào outbox; thêm regression. |
