...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] QA Review` | **Đã sửa theo QA 2026-07-23:** Luồng ingress được tách thành `classifyTuningMessage` → reserve terminal report → validate/persist/dispatch → finalize report; đường không có root UUID canonical fail-closed bằng disconnect/redelivery, không silent drop. Outbox không có bản sao state theo `UNIT_TEST`; chỉ giải phóng sau transport xác nhận QoS-1. Test contract độc lập bao phủ malformed/oversize/missing-invalid UUID, canonical escaped root key, outbox full không mutation, reconnect và ACK lifecycle. |
