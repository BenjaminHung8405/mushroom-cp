...
| C5 | Implement đọc/ghi NVS two-slot: verify CRC/readback, chọn generation mới nhất, wear-level slot và fallback defaults. | `[ ] QA Review` | **QA REJECTED 2026-07-21:** `saveDurableReceipt()` mới chỉ kiểm tra số byte ghi, chưa readback/CRC; lỗi hoặc corruption sau write có thể làm mất duplicate identity sau reboot. Phải verify record receipt đọc lại đầy đủ (version, UUID, CRC) trước khi cập nhật RAM và thêm fault-injection regression. |
...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] QA Review` | **QA REJECTED 2026-07-21:** Nhánh publish QoS-1 đầu tiên vẫn xóa `pendingQos1` khi `writePendingQos1()` thất bại, làm mất reported ACK; phải giữ packet đến reconnect/retry và thêm regression cho first-send transport failure. |
