...
|---|---|---|---|
| A1 | Định nghĩa constants cho MQTT tuning desired/reported và pattern subscribe; tenant lấy từ `IOT_TENANT`. | `[ ] QA Review` | **QA REJECTED 2026-07-23:** Loại bỏ fallback hard-code tenant `mushroom` khỏi runtime MQTT/ACL; tenant phải được cấu hình bắt buộc qua `IOT_TENANT` và fail closed khi thiếu. |
...
| C5 | Implement đọc/ghi NVS two-slot: verify CRC/readback, chọn generation mới nhất, wear-level slot và fallback defaults. | `[ ] QA Review` | **QA REJECTED 2026-07-23:** Di chuyển NVS/Preferences implementation khỏi `src/core/` sang storage layer (core không được phụ thuộc NVS API), giữ core API/domain thuần và cập nhật test. |
...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] QA Review` | **QA REJECTED 2026-07-23:** Không được drop reported ACK khi FIFO QoS 1 đầy; bổ sung retry/durable outbox hoặc cơ chế back-pressure để ACK được giữ đến khi PUBACK/reconnect. |
