...
| C5 | Implement đọc/ghi NVS two-slot: verify CRC/readback, chọn generation mới nhất, wear-level slot và fallback defaults. | `[ ] In Progress` | **QA REJECTED 2026-07-23:** `verifyReadback()` chỉ so CRC/generation/commit_state, không so toàn bộ record với `expected`; một record bị sửa field nhưng được tính lại CRC vẫn được chấp nhận. Phải dùng `memcmp` fixed-size (hoặc so sánh đầy đủ mọi byte persisted) sau khi `isValidRecord()`, và sửa regression để thực sự chứng minh mutation có CRC hợp lệ bị `REJECTED/NVS_WRITE_ERROR`. |
...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[x] Done` | QA approved 2026-07-23: artifact test nhị phân đã bị xóa khỏi Git tree và được ignore; luồng QoS 1 giữ packet pending qua transport failure/reconnect và kiểm tra PUBACK sai ID. |
