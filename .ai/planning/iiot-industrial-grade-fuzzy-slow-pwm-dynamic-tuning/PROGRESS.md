...
| C5 | Implement đọc/ghi NVS two-slot: verify CRC/readback, chọn generation mới nhất, wear-level slot và fallback defaults. | `[ ] QA Review` | **QA REJECTED 2026-07-21:** Bỏ nhánh `UNIT_TEST` làm vô hiệu so sánh CRC readback với record vừa ghi; test phải kiểm tra cùng fail-closed invariant như firmware production. |
...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] QA Review` | **QA REJECTED 2026-07-21:** FIFO QoS-1 loại packet khỏi queue trước khi write thành công; lỗi transport làm mất reported ACK. Phải giữ packet cho reconnect/retry và thêm regression cho lỗi write/reconnect. |
