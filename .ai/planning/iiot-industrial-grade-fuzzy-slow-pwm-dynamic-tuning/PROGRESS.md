...
| Task ID | Mô tả Task | Status | Note / chỉ thị kỹ thuật bắt buộc |
|---|---|---|---|
| F1 | Tạo migration `1720656000006` cho `device_tuning_configurations` và index theo device/thời gian. | `[ ] QA Review` | Migration là **nguồn sự thật duy nhất** cho schema; reversible `down`, FK đúng `devices(device_id)`, index phục vụ latest lookup. UUID tạo tại application nếu không bảo đảm `pgcrypto`; không sửa migration cũ. **QA REJECTED 2026-07-24:** thiếu unique idempotency constraint `(device_id, command_id)`; cần xử lý race revision/idempotency. |
| F2 | Tạo migration `1720656000007` cho `tuning_audit_logs` và index theo device/thời gian. | `[ ] QA Review` | Audit append-only; lưu actor/source/action/ruleset/KPI/before/after/reason/result. Bảo toàn referential integrity, không lưu secrets/credential trong JSONB; migration phải chạy clean database và upgrade database. **QA REJECTED 2026-07-24:** FK `configuration_id ... ON DELETE CASCADE` cho phép xóa audit history, trái append-only. |
| F3 | Khai báo entity `DeviceTuningConfiguration`, `TuningConfigSnapshot` và `SyncStatus`. | `[ ] QA Review` | Áp dụng **domain model rõ kiểu**; JSONB snapshot phải là full effective config, không patch. Mapping column/relationship khớp migration; strict nullability và enum không dùng string rải rác. |
| F4 | Khai báo entity `TuningAuditLog`. | `[ ] QA Review` | Entity chỉ phản chiếu migration, không dùng synchronize schema. Audit không được sửa/xóa bởi normal service flow; truy vấn luôn theo device và phân trang. |
| F5 | Implement `handleReportedAck()` với type guard, transaction/row lock, canonical comparison, state transition, audit và SSE sau commit. | `[ ] QA Review` | Áp dụng **transactional outbox discipline**: `SELECT ... FOR UPDATE`, idempotent khi ACK QoS-1 lặp; chỉ phát `tuningSync$` sau commit. ACK unknown/sai device chỉ security log, không mutate shadow hay clear retained. Raw DB error phải chuyển thành domain exception/log an toàn. **QA REJECTED 2026-07-24:** không kiểm `persisted=true` trước transition `IN_SYNC`; clear retained không có retry durable. |
| F6 | Implement `createPendingCommand()` tạo desired/audit, publish desired retained QoS 1 và ghi thời điểm publish. | `[ ] QA Review` | Actor phải từ JWT đã verify, không từ client. Dùng command idempotency key, full config snapshot và ownership check. Làm rõ xử lý lỗi publish (không được để row PENDING mồ côi không audit/retry strategy); không auto-apply. **QA REJECTED 2026-07-24:** bounds trái contract v1, actor/ownership không được xác minh, và retry idempotency có thể publish config client khác DB shadow. |
