...
| Task ID | Mô tả Task | Status | Note / chỉ thị kỹ thuật bắt buộc |
|---|---|---|---|
| F1 | Tạo migration `1720656000006` cho `device_tuning_configurations` và index theo device/thời gian. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** `0006` giữ nguyên; delta hardening/reported đưa vào `0008/0010` với preflight fail-before-DDL và integration test PostgreSQL thật (clean install/rollback + upgrade duplicate/clean). |
| F2 | Tạo migration `1720656000007` cho `tuning_audit_logs` và index theo device/thời gian. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** `0007` giữ nguyên; FK audit chuyển `RESTRICT` qua `0008` và có rollback đối xứng khôi phục `ON DELETE CASCADE`; index outbox partial due được thêm ở `0009`. |
| F3 | Khai báo entity `DeviceTuningConfiguration`, `TuningConfigSnapshot` và `SyncStatus`. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** entity persist `reported_config/reported_revision/applied_at/rejection_reason`; migration `0010` thêm cột + index `(device_id, revision DESC)`. |
| F4 | Khai báo entity `TuningAuditLog`. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** FK `RESTRICT`, audit ghi `configBefore/configAfter` từ reported canonical hoặc desired trước đó; append-only về mặt service (không có update/delete). |
| F5 | Implement `handleReportedAck()` với type guard, transaction/row lock, canonical comparison, state transition, audit và SSE sau commit. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** `ackRejectionReason()` fail-closed (`PERSISTENCE_NOT_CONFIRMED`, `REVISION_MISMATCH`, `CANONICAL_MISMATCH`, `EDGE_REJECTED`); IN_SYNC chỉ khi persisted+revision+config canonical trùng desired; retained clear qua outbox, chỉ dispatch nếu vẫn là latest. |
| F6 | Implement `createPendingCommand()` tạo desired/audit, publish desired retained QoS 1 và ghi thời điểm publish. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** transactional outbox durable, `supersedeUndeliveredDesired()` mark delivered mọi revision cũ chưa publish; desired payload mang `schema_version=1`+`revision`; firmware fence `STALE_REVISION`. |
| F7 | Implement `getLatestByDeviceId()` với latest durable shadow. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** query dùng `revision DESC`, hỗ trợ bởi index `idx_device_tuning_configs_device_revision`. |
| F8 | Implement `getTuningHistory()` với phân trang. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** pagination fail-closed thành `NaN` khi input không phải số; guard JWT + `assertReadAccess()` kiểm house ownership trước mỗi read. |
| F9 | Khai báo `TuningModule`, import dependencies, export service và import vào `AppModule`. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** module xuất `TuningConfigurationService`, đăng ký `TuningMqttOutboxDispatcher` + `TuningPrincipalGuard`; import forwardRef MqttModule, không có circular unsafe. |
| F10 | MqttService subscribe wildcard reported QoS 1, type-guard payload và route tới `TuningConfigurationService`. | `[x] Done` | **QA APPROVED 2026-07-24 (vòng 4):** `parseTuningSnapshot` strict v1 (đúng 4 key, finite, bounds, gap `0.001`); `handleTuningReported` drop malformed + `device_id` mismatch trước khi lên bus; `publishTuningDesired` mang `revision`. |
