...
| D4 | Xây reported payload theo contract và publish lên reported topic QoS 1, retain=false. | `[ ] QA Review` | **QA REJECTED 2026-07-24 — đã khắc phục, chờ QA Review lần 2:** `mqtt_manager.cpp` nay trích xuất bounded root `command_id` canonical trước full parse, reserve outbox trước mutation và finalize `REJECTED/INVALID_SCHEMA` cho payload malformed/oversize có identity. Callback chuyển tiếp oversize bounded đến worker; chỉ disconnect/redelivery khi không thể định danh ACK hoặc outbox đầy. Regression qua `processNetworkMessage()` kiểm tra terminal ACK, không mutation, không disconnect và giữ ACK đến QoS-1 completion. |
