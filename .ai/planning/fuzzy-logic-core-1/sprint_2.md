...
### 2.1 PHỤ THUỘC AN TOÀN CORE 1 — NTP TIME PROVIDER
Sau khi WiFi Station có kết nối, Core 0 phải khởi tạo/duy trì đồng bộ NTP múi giờ UTC+7. Core 1 chỉ được nhận giờ qua API thread-safe hoặc system clock thread-safe để tạo `RtcTimePod`; thời gian chưa đồng bộ hoặc không còn hợp lệ phải được biểu diễn rõ bằng `valid=false`, không được dùng giờ mặc định. Đây là điều kiện để hard interlock 11:00–13:30 hoạt động đúng và fail-safe.
...
- **Tạo file:** `NetworkTask.h` / `NetworkTask.cpp`
  ...
  - *NTP time provider*: Khi WiFi Station kết nối, gọi `configTime(7 * 3600, 0, ...)` và kiểm tra thời gian đã đồng bộ trước khi công bố là hợp lệ cho Core 1. Mất WiFi sau khi đã sync không đồng nghĩa ngay với invalid, nhưng phải có chính sách timeout/validity rõ ràng; chưa sync lần nào tuyệt đối không được trả giờ giả.
...
- **Tạo/Sửa file:** `main.cpp`
  - *Hàm `setup()`*: Khởi tạo GPIO fail-safe, I2C, kích hoạt `initWiFiModes()`, cấu hình NTP khi Station sẵn sàng, rồi `xTaskCreatePinnedToCore()` gọi Core1Task. Core 1 phải nhận `RtcTimePod{false,...}` cho đến khi NTP xác thực thành công.
