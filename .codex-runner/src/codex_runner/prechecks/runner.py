...
from codex_runner.models import CommandResult, Precheck

class PrecheckRunner:
    def __init__(self, checks: list[Precheck]): self.checks = checks
    def run(self, cwd: Path, dry_run: bool = False) -> list[CommandResult]:
        ...
        for check in self.checks:
            ...
            try:
                p = subprocess.run(command, cwd=cwd, text=True, capture_output=True, check=False, timeout=check.timeout)
                print(p.stdout, end=""); print(p.stderr, end="")
                results.append(CommandResult(command, p.returncode, p.stdout, p.stderr))
            except subprocess.TimeoutExpired as exc:
                results.append(CommandResult(command, -1, exc.stdout or "", exc.stderr or "", True))
