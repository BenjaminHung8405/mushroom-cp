...
## [2026-08-02T22:11:01+07:00]
- [Refactor] Thay đổi tham số hàm `send2Displ` để nhận 2 tham số `v1`, `v2` thay vì chuỗi.
- Tự động lấy tối đa 2 chữ số cuối (phép tính modulo 100) cho mỗi giá trị truyền vào.
- Nếu giá trị nhỏ hơn 10 (chỉ có 1 chữ số), tự động sử dụng khoảng trắng làm đệm ở hàng chục (thay vì số '0'). Cách xử lý này luôn đảm bảo không có 2 giá trị khoảng trắng (0xFF) liền kề trong chuỗi 4 chữ số gốc bất kể cấu hình mode hiển thị nào đang được sử dụng.
- Cập nhật lại mã nguồn trong hàm `setup()` và `loop()` của chương trình chính để tương thích với cách gọi mới.

## [2026-08-02T21:34:19+07:00]
- [Feature] Bổ sung cấu hình `I2C_DISPLAY_MODE` (nằm chung nhóm với `I2C_DISPLAY_ADDR` trong `config.h`) để tùy chỉnh cách hiển thị lên màn hình I2C.
  - Mode 0: Padding khoảng trắng bên trái (VD: '1234' -> `   1234`).
  - Mode 1: Đảo ngược chuỗi số và padding khoảng trắng bên phải (VD: '1234' -> `4321   `).
- Cập nhật hàm logic `formatDispData` và mở rộng các test cases tương ứng trong `test_formatDispData` (áp dụng triệt để Poka-Yoke & Testability).

## [2026-08-02T21:07:48+07:00]
- [Feature] Bổ sung logic hiển thị bộ đếm biến tăng dần (non-blocking timer) ra màn hình I2C trong `loop()` của chương trình chính. Bộ đếm `tTest` khởi đầu ở `1111111` và tự động tăng 1 đơn vị mỗi giây, đồng thời gửi ra màn hình thông qua `send2Displ` và xuất trạng thái qua Serial (thông qua `LOG_PRINTF`).

## [2026-08-02T16:50:31+07:00]
- [Feature] Khởi tạo bus I2C trong `setup()` của chương trình chính (`mushroom-cp-firmware.ino`) bằng `Wire.begin(...)` với cấu hình chân được định nghĩa sẵn. Bổ sung gọi thử hàm `send2Displ("1234")` để kiểm tra màn hình ngay khi khởi động.

## [2026-08-02T16:44:36+07:00]
- Tạo mới module `dispi2c` (`dispi2c.h` và `dispi2c.cpp`) để quản lý việc truyền dữ liệu hiển thị qua I2C.
- Áp dụng nguyên tắc Poka-Yoke và LEAN bằng cách tách biệt logic xử lý chuỗi (`formatDispData`) ra khỏi hàm ghi thiết bị (`send2Displ`). Việc này giúp có thể Unit Test phần logic.
- Bổ sung test case `test_formatDispData()` ngay trong module để xác minh tính đúng đắn của logic xử lý chuỗi và padding.

## [2026-08-02T16:14:20+07:00]
- Bổ sung định nghĩa địa chỉ I2C cho màn hình (`I2C_DISPLAY_ADDR = 0x51`) vào file `config.h`.
