  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const deviceId =
      typeof request.params?.id === 'string'
        ...
        : typeof request.params?.deviceId === 'string'
          ...
          : null;
    const houseId =
      typeof request.params?.houseId === 'string'
    ...
    if (deviceId) {
      const rows = (await this.dataSource.query(
        'SELECT house_id FROM devices WHERE device_id = $1',
        [deviceId],
      )) as Array<{ house_id: string }>;
      if (!rows.length) throw new NotFoundException('Resource not found.');
      if (!principal.houseIds.includes(rows[0].house_id))
      ...
      if (request.method !== 'GET')
        throw new ForbiddenException('A scoped batch request is required.');
    }
    return true;
  }
}
