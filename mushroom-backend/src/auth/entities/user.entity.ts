...
export enum UserRole { ADMIN = 'ADMIN', OPERATOR = 'OPERATOR', AUDITOR = 'AUDITOR' }
...
export class User {
  ...
  @Column({ name: 'pin_failed_attempts', default: 0 }) pinFailedAttempts!: number;
  /** If set and in the future, login is temporarily blocked */
  @Column({ name: 'pin_locked_until', type: 'timestamptz', nullable: true }) pinLockedUntil!: Date | null;
