import { BadRequestException, Controller, Get, HttpException, HttpStatus, Param, Query } from '@nestjs/common';
...
export class OfflineSyncController {
  ...
  async getHistory(
    ...
    if (!deviceId || Number.isNaN(fromDate.getTime()) || Number.isNaN(toDate.getTime()) || fromDate >= toDate) {
      throw new BadRequestException('deviceId, valid from, and valid to (from < to) are required');
    }
