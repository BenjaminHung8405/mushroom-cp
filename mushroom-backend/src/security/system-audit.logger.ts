...
const SECRET_KEY_PATTERN = /authorization|token|password|secret|cookie|credential/i;
...
function boundedPayload(body: unknown): { payload: Record<string, unknown> | null; hash: string | null } {
  ...
  return {
    payload: { _truncated: true, preview: serialized.slice(0, MAX_SERIALIZED_PAYLOAD_BYTES) },
...
export class SystemAuditLogger implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const persist = (statusCode: number, result: string) => from(
    ...
    return next.handle().pipe(
      mergeMap((value) => persist(response.statusCode, 'SUCCESS').pipe(map(() => value))),
      catchError((error: unknown) => {
        void persist(response.statusCode >= 400 ? response.statusCode : 500, 'FAILURE').subscribe();
        throw error;
      }),
    );
  }
