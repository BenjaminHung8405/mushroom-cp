...
describe('MqttAuthService', () => {
  ...
  it('authenticates an enabled device only when its client id and token match', async () => {
    ...
    await expect(
      service.authenticate({
        username: 'mushroom_s3_aabbccddeeff',
        clientid: 'mushroom_s3_aabbccddeeff',
  ...
  it('enforces bootstrap and device ACL boundaries', () => {
    expect(
      service.authorize({
        username: 'provision_node',
        clientid: 'aabbccddeeff',
        topic: 'mushroom/provision/request',
        acc: 2,
      }),
    ).toBe(true);
    expect(
      service.authorize({
        username: 'provision_node',
        clientid: 'aabbccddeeff',
        topic: 'mushroom/provision/response/aabbccddeeff',
        acc: 1,
      }),
    ).toBe(true);
    expect(
      service.authorize({
        username: 'provision_node',
        clientid: 'aabbccddeeff',
        topic: 'mushroom/esp32/other/up/telemetry',
        acc: 2,
      }),
    ).toBe(false);
    expect(
      service.authorize({
        username: 'mushroom_s3_aabbccddeeff',
        clientid: 'mushroom_s3_aabbccddeeff',
        topic: 'mushroom/esp32/mushroom_s3_aabbccddeeff/up/telemetry',
        acc: 2,
      }),
    ).toBe(true);
  });
  ...
  it('limits provisioning requests independently per MAC', () => {
    for (let i = 0; i < 5; i += 1)
      service.enforceProvisionRateLimit('aabbccddeeff');
    expect(() => service.enforceProvisionRateLimit('aabbccddeeff')).toThrow(
