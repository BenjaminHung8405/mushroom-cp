/* eslint-disable @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-return, @typescript-eslint/require-await -- Jest transaction callbacks execute typed test doubles. */
import {
  BadRequestException,
  ConflictException,
  ForbiddenException,
...
describe('TuningConfigurationService hardening', () => {
  ...
  let configRepo: jest.Mocked<
    Pick<
      Repository<DeviceTuningConfiguration>,
      'find' | 'findOne' | 'save' | 'update'
    >
  >;
  let mqtt: {
    publishTuningDesired: jest.Mock;
    clearTuningDesired: jest.Mock;
    tuningReported$: Subject<TuningReportedEvent>;
  };
  let outbox: {
    enqueueDesired: jest.Mock;
    enqueueRetainedClear: jest.Mock;
    dispatchDue: jest.Mock;
  };
  ...
  beforeEach(async () => {
    ...
    mqtt = {
      ...
      clearTuningDesired: jest.fn(),
      tuningReported$: new Subject<TuningReportedEvent>(),
    };
    outbox = {
      enqueueDesired: jest.fn(),
      enqueueRetainedClear: jest.fn(),
      dispatchDue: jest.fn(),
      supersedeUndeliveredDesired: jest.fn(),
    };
  ...
  it('enforces immutable v1 bounds before database mutation', async () => {
    ...
    await expect(
      service.createPendingCommand(
        principal,
        deviceId,
        { ...config, mist_off_threshold: 0.21 },
        commandId,
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('rejects a principal outside the device house scope in the write transaction', async () => {
    dataSource.transaction.mockImplementation(async (callback) =>
      callback({
        ...
        findOne: jest
          .fn()
          .mockResolvedValue({ deviceId, houseId: 'house-1', enabled: true }),
      }),
    );
    await expect(
      service.createPendingCommand(
        { subject: 'other', allowedHouseIds: [] },
        deviceId,
        config,
        commandId,
      ),
  ...
  it('re-checks current ownership inside the write transaction before creating a command', async () => {
    const saved = {
      id: 'config-1',
      deviceId,
      commandId,
    ...
    const manager = {
      ...
      findOne: jest
        ...
        .mockImplementation((entity: unknown) =>
          entity === Device
            ? { deviceId, houseId: 'house-1', enabled: true }
            : null,
        ),
    ...
    await expect(
      service.createPendingCommandByOwner(
        'owner-1',
        'owner@example.com',
        deviceId,
        config,
    ...
    expect(manager.create).toHaveBeenCalledWith(
      TuningAuditLog,
      expect.objectContaining({ actor: 'owner@example.com' }),
    );
  });

  it('rejects a command when ownership changes after the guard but before the transaction', async () => {
    const manager = {
      query: jest
        .fn()
    ...
    expect(outbox.enqueueDesired).not.toHaveBeenCalled();
  });

  it('returns 409 for a duplicate commandId with a different snapshot without new audit or outbox rows', async () => {
    const existing = {
      id: 'config-1',
      deviceId,
      commandId,
    ...
    expect(manager.save).not.toHaveBeenCalled();
    expect(outbox.enqueueDesired).not.toHaveBeenCalled();
    expect(outbox.supersedeUndeliveredDesired).not.toHaveBeenCalled();
  ...
  it('uses the immutable persisted snapshot for an unpublished idempotent command', async () => {
    ...
    configRepo.save.mockImplementation(
      async (value) => value as DeviceTuningConfiguration,
    );
    await service
  ...
  it.each([
    ...
    'drops terminal REJECTED ACK with %s before transaction, audit, SSE, or retained-clear',
  ...
  it('routes the same terminal REJECTED ACK from the MQTT event stream to durable state, audit, and SSE', async () => {
    ...
    expect(manager.create).toHaveBeenCalledWith(
      TuningAuditLog,
      expect.objectContaining({ action: 'SYNC_REJECTED' }),
    );
    expect(events).toHaveLength(1);
  });
  ...
  it('rejects an accepted ACK whose revision or canonical config mismatches the desired', async () => {
    ...
    await service.handleReportedAck({
      deviceId,
      commandId,
      status: 'ACCEPTED',
      persisted: true,
    ...
    const manager2 = {
      findOne: jest
        ...
        .mockResolvedValueOnce(other)
        .mockResolvedValueOnce(null),
      save: jest.fn().mockResolvedValue(other),
      create: jest.fn((_entity: unknown, value: unknown) => value),
    };
    dataSource.transaction.mockImplementation(async (callback) =>
      callback(manager2),
  ...
  it('queues a retained clear durably instead of issuing MQTT from the ACK transaction', async () => {
    ...
    const manager = {
      findOne: jest
        ...
        .mockResolvedValueOnce(synced)
        .mockResolvedValueOnce(null),
      save: jest.fn().mockResolvedValue(synced),
      create: jest.fn((_entity: unknown, value: unknown) => value),
    };
    dataSource.transaction.mockImplementation(async (callback) =>
      callback(manager),
  ...
  it('accepts the durable no-change revision after reboot/replay exactly once without a revision mismatch', async () => {
    ...
    await expect(service.handleReportedAck(replayAck)).resolves.toEqual({
      updated: true,
      isLatest: true,
    });
    await expect(service.handleReportedAck(replayAck)).resolves.toEqual({
